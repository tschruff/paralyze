...
class UtilsTests(TestCase):
    ...
    def test_nested_dict(self):
        ...
        self.assertEqual(len(nd.keys()), 5)
        self.assertEqual(len(nd.values()), 5)
        self.assertEqual(len(nd.items()), 5)

        self.assertEqual(len(nd.leaf_keys()), 3)
        self.assertEqual(len(nd.leaf_values()), 3)
        self.assertEqual(len(nd.leaf_items()), 3)
    ...
    def test_nested_config(self):
        ...
        values = config.finalize(x=3, variables={'x': 1, 'y': 2})
        ...
        values = config.finalize(x=6, variables={'x': 1, 'y': 2})
        ...
        values = config.finalize(x=1, variables={'x': 1, 'y': 2})
        ...
        self.assertEqual(int(values['c'][0]), 1)
        self.assertEqual(int(values['c'][1]), 2)
